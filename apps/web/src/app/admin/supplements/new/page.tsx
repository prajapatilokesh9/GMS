...
import { getSupplementCompanies, createSupplement } from '@/lib/api';
...
function CreateSupplementContent() {
  ...
  async function loadCompanies() {
    try {
      ...
      setCompanies(list.filter((c: Company) => c.isActive));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err.response?.data?.error?.message || err.response?.data?.message || 'Failed to create supplement');
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <main className="flex-1 overflow-auto bg-gray-50 p-8">
        <div className="mx-auto max-w-2xl space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border bg-white p-6 shadow-sm">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Supplement Name *</label>
