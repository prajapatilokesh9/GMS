...
function CreateTrainerContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err.response?.data?.error?.message || err.response?.data?.message || 'Failed to create trainer');
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <div className="mx-auto max-w-2xl space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border bg-white p-6 shadow-sm">
            <div>
              ...
              <input value={userId} onChange={(e) => setUserId(e.target.value)} required placeholder="Enter user UUID"
            ...
            <div>
              ...
              <input value={specialization} onChange={(e) => setSpecialization(e.target.value)} placeholder="e.g., Strength Training, Yoga"
                className="mt-1 w-full rounded border px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
            </div>
            <div>
