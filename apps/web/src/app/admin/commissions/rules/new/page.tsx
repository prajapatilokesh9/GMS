...
import { createCommissionRule, getGyms, getTrainers } from '@/lib/api';
...
interface Trainer { id: string; user: { id: string; email: string; firstName: string; lastName: string }; }
...
function CreateRuleContent() {
  ...
  useEffect(() => {
    ...
    if (user) { loadGyms(); loadTrainers(); }
  }, [user, authLoading]);

  async function loadGyms() { try { const d = await getGyms(); setGyms(Array.isArray(d) ? d : []); } catch {} }
  async function loadTrainers() { try { const d = await getTrainers(); setTrainers(Array.isArray(d) ? d : []); } catch {} }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err.response?.data?.error?.message || err.response?.data?.message || 'Failed to create rule');
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 overflow-auto bg-gray-50 p-8">
        <div className="mx-auto max-w-2xl space-y-6">
          <h1 className="text-2xl font-bold">New Commission Rule</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border bg-white p-6 shadow-sm">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Trainer (optional — leave empty for gym-wide rule)</label>
              <select value={trainerId} onChange={(e) => setTrainerId(e.target.value)}
                className="mt-1 w-full rounded border px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
                <option value="">All Trainers (gym-wide)</option>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">{commissionType === 'percentage' ? 'Percentage (%)' : 'Fixed Amount (₹)'} *</label>
              <input type="number" step="0.01" min="0" value={commissionValue} onChange={(e) => setCommissionValue(e.target.value)} required
                className="mt-1 w-full rounded border px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
            </div>
            <div>
